...
pub fn parse_inspection(text: &str) -> anyhow::Result<InspectionFunction> {
    ...
    let mut tokens = function.trim().split_ascii_whitespace();
    let left = tokens
        .next()
    ...
    Ok(match left {
        "old" => match right {
            "old" => Box::new(move |var| (*op)(var, var)),
            r_str => {
                let r_value: u64 = r_str.parse()?;
                Box::new(move |var| (*op)(var, r_value))
            }
        },
        l_str => {
            let l_value: u64 = l_str.parse()?;
            match right {
                "old" => Box::new(move |var| (*op)(l_value, var)),
                r_str => {
                    let r_value: u64 = r_str.parse()?;
                    Box::new(move |_var| (*op)(l_value, r_value))
                }
            }
        }
    })
